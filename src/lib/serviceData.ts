...
export function getSavedServices(): RepairService[] {
  ...
  try {
    ...
    const allowedUsed = ["Used iPhone", "Used Samsung", "Used Android", "Used iPad", "Used Laptop", "Other / Custom Repair"];
    ...
    // Sync hidden state for default unhidden items so existing users get the fix!
    let hasChangedVisibility = false;
    filtered = filtered.map(item => {
      const isDefaultUnhidden = DEFAULT_UNHIDDEN_NAMES.some(defName => 
        defName.toLowerCase() === item.name.toLowerCase()
      );
      if (isDefaultUnhidden && item.hidden) {
        item.hidden = false;
    ...
    // Ensure all 5 allowed 'Used' services + 'Other / Custom Repair' are present
    const existingNames = new Set(filtered.map(s => s.name));
    const usedServicesToAdd = REPAIR_SERVICES.filter(s => s.category === 'other' || s.name.startsWith('Used ') || s.name === "Other / Custom Repair");
    let hasAdded = false;
    ...
    if (hasAdded || hasChangedVisibility || filtered.length !== parsed.length) {
