...
interface AIPanelLeadsFeedProps {
  ...
  onSendReply: (id: string, message: string) => Promise<any>;
}
...
export function AIPanelLeadsFeed({
  ...
  const handleSendReply = async (leadId: string) => {
    ...
    setReplyError(null);
    try {
      const result = await onSendReply(leadId, replyText.trim());
      if (result && result.emailSent === false && result.emailError) {
        setReplyError(`Saved, but email not sent: ${result.emailError}`);
      } else {
        setReplyText('');
                ...
                >
                  ...
                  {lead.replies && lead.replies.length > 0 && (
                    <div className="space-y-1.5">
                      {lead.replies.map((reply: any) => (
                        <div key={reply.id} className="bg-blue-600 text-white text-[10px] p-2 rounded-xl rounded-tr-sm ml-6">
                          {reply.message}
                        </div>
                      ))}
                    </div>
                  ...
                  {isReplying && (
                    ...
                    >
                      ...
                      />
                      {replyError && (
                        <p className="text-[9px] font-bold text-amber-600">{replyError}</p>
                      )}
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          ...
                          className={cn(
                            "px-3 py-1 text-[9px] font-bold uppercase tracking-wider text-white transition-all rounded-lg flex items-center gap-1 disabled:opacity-40",
